...
**Completed output:** watch the [published 1:57 demonstration](https://youtu.be/GGgn0UvC4n0). It shows the venue-change workflow and a later intentional arrival-time difference. The [edit record](VIDEO_EDIT.md) describes the actual footage, timeline, and evidence boundaries. The instructions below are a reusable capture workflow; suggested take names and timing are preparation guidance, not a claim that the final film followed every planned shot.
...
4. Show the matching Doc and Slack baseline, then send the take's confirmed venue-change message yourself. Return directly to the affected Doc or Gmail composer and keep recording until the notice appears. The published recording returns to Docs first. Do not open a comparison chat or manually enroll sources to trigger it.
...
- Keep the **Slack change → return to the affected application → automatic notice** causal sequence clear. Label shortened waits, for example **Waiting accelerated**. Do not splice a notice or success receipt from an unrelated take into that sequence or present edited duration as measured latency. Label continuation footage explicitly.
...
The local FFmpeg editing pipeline has been tested. The entrant supplied four real recordings and published the completed v2 demo. It is 1:57 and uses English male AI narration, matching the entrant's voice preference. It includes an intentional-difference explanation, a revised arrival-time sentence, and its successful save. The earlier 1:55 v1 remains available locally. This is a generated narrator, not a recording or clone of the entrant's voice. See [the demo edit record](VIDEO_EDIT.md) for the actual timeline and editing decisions.
